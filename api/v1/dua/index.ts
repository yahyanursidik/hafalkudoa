...
import { listActiveDua } from "../../../src/api/dua.js";
...
export default async function handler(request: VercelRequest, response: VercelResponse): Promise<void> {
  ...
  try {
    ...
    response.status(200).json(await listActiveDua(getDatabase()));
  } catch (error) {
    const formatted = toErrorResponse(error, id);
    logger.error("dua_list_failed", { requestId: id, statusCode: formatted.statusCode });
